...
namespace Classes {
  public class MediaFile {
    ...
    private MediaStream[] _Streams => this._streams ?? (this._streams = _GetStreams(_GetSections(this._GatherCommandlineInfo())).ToArray());
    ...
    private static IEnumerable<MediaStream> _GetStreams(IEnumerable<Tuple<string, SectionDictionary>> sections) {
      foreach (var tuple in sections) {
        switch ((tuple.Item1 + " ").Split(new[] { ' ' }, 2)[0]) {
          case "General":
          {
    ...
    public static MediaFile FromFile(FileInfo file) => new MediaFile(file);
