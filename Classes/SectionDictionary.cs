...
namespace Classes {
  internal class SectionDictionary {
    ...
    public string GetValueOrDefault(string key, int index = 0, string defaultValue = default(string)) => this._values.GetValueOrNull(key)?.Skip(index).FirstOrDefault() ?? defaultValue;
  }
}